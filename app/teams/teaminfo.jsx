...
import { View, Text, Modal, ScrollView , TouchableOpacity, StyleSheet, ActivityIndicator, Dimensions } from 'react-native';
                        ...
                        {chunkedPast.map((group, groupIdx) => (
                          <View key={`past-${groupIdx}`} style={styles.gameRow}>
                            {group.map((item, idx) => {
                              ...
                              return (
                                <View key={idx} style={{marginVertical: 10, alignItems: 'center', width: 50}}>
                                  <Text style={[styles.gameOppText, { backgroundColor: bgColor, color: '#242424', fontSize: 12, fontWeight: 800 }]}>
                                    {item.opponent}
                                  </Text>
